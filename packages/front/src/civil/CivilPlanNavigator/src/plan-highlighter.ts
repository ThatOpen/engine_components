...
export class PlanHighlighter {
  ...
  private readonly markupMaterial = new THREE.LineBasicMaterial({
    color: 0x686868,
  });
  ...
  private showLineInfo(curveMesh: FRAGS.CurveMesh, offset: number) {
    ...
    marker.showLineLength(lineParallelLine, curveMesh.curve.getLength());
    this._scene.add(lineParallelLine);
  ...
  private showClothoidInfo(curveMesh: FRAGS.CurveMesh, offset: number) {
    ...
    marker.showCurveLength(parallelCurvePoints, curveMesh.curve.getLength());
    const clothParallelLine = new THREE.Line(
      lengthGeometry,
      this.markupMaterial,
    );
    this._scene.add(clothParallelLine);
  ...
  private showCircularArcInfo(curveMesh: FRAGS.CurveMesh, offset: number) {
    ...
    marker.showCurveRadius(radiusLine, Math.abs(radius));
    this._scene.add(radiusLine);
    ...
    this._scene.add(circArcParallelLine);
