...
export class IfcPropertiesFinder extends Component implements Disposable {
  readonly onFound = new Event<FRAGS.FragmentIdMap>();
  ...
  private onFragmentsDisposed = (data: {
    ...
    delete this.indexedModels[data.groupID];
  };
  ...
  private async indexEntityRelations(model: FragmentsGroup) {
    ...
    this.indexedModels[model.uuid] = map;
    return map;
  }

  async find(groups: QueryGroup[], queryModels?: FragmentsGroup[]) {
    const fragments = this.components.get(FragmentManager);

    const models = queryModels || fragments.groups.values();
    const result: QueryResult = {};

    for (const model of models) {
      let map = this.indexedModels[model.uuid];
      if (!map) {
  ...
  private toFragmentMap(data: QueryResult) {
    const fragments = this.components.get(FragmentManager);
    const fragmentMap: FRAGS.FragmentIdMap = {};
