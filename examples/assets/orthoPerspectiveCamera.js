import{p,m as d,a as m,b}from"./index-BNf8AL5k.js";import{C as u,W as h,S as f,a as g,O as w,F as v}from"./index-DwlKX12C.js";import{G as k}from"./index-DcEouUvL.js";const r=new u,P=r.get(h),t=P.create();t.scene=new f(r);t.scene.setup();t.scene.three.background=null;const F=document.getElementById("container");t.renderer=new g(r,F);t.camera=new w(r);await t.camera.controls.setLookAt(68,23,-8.5,21.5,-5.5,23);r.init();const O=r.get(k).create(t),j="https://thatopen.github.io/engine_fragment/resources/worker.mjs",y=await fetch(j),U=await y.blob(),C=new File([U],"worker.mjs",{type:"text/javascript"}),L=URL.createObjectURL(C),n=r.get(v);n.init(L);t.camera.controls.addEventListener("update",()=>n.core.update());t.onCameraChanged.add(e=>{for(const[,o]of n.list)o.useCamera(e.three);n.core.update(!0)});n.list.onItemSet.add(({value:e})=>{e.useCamera(t.camera.three),t.scene.three.add(e.object),n.core.update(!0)});n.core.models.materials.list.onItemSet.add(({value:e})=>{"isLodMaterial"in e&&e.isLodMaterial||(e.polygonOffset=!0,e.polygonOffsetUnits=1,e.polygonOffsetFactor=Math.random())});const S=["https://thatopen.github.io/engine_components/resources/frags/school_arq.frag"];await Promise.all(S.map(async e=>{var l;const o=(l=e.split("/").pop())==null?void 0:l.split(".").shift();if(!o)return null;const i=await(await fetch(e)).arrayBuffer();return n.core.load(i,{modelId:o})}));t.camera.projection.onChanged.add(()=>{const e=t.camera.projection.current;O.fade=e==="Perspective"});p.init();const c=d.create(()=>m`
  ...
  `);document.body.append(c);const I=d.create(()=>m`
    ...
    `);document.body.append(I);const a=new b;a.showPanel(2);document.body.append(a.dom);a.dom.style.left="0px";a.dom.style.zIndex="unset";t.renderer.onBeforeUpdate.add(()=>a.begin());t.renderer.onAfterUpdate.add(()=>a.end());
