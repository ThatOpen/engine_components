import{l as C,c as g,a as m,b as y,V as u}from"./index-D_lOu8Cy.js";import{C as V,W as S,S as v,a as I,O as k,F as P,V as $,B as U,H as f}from"./index-BXecBxnk.js";import{I as D}from"./index-BnfRX7vA.js";const i=new V,M=i.get(S),o=M.create();o.scene=new v(i);o.scene.setup();o.scene.three.background=null;const R=document.getElementById("container");o.renderer=new I(i,R);o.camera=new k(i);await o.camera.controls.setLookAt(78,20,-2.2,26,-4,25);i.init();const L="https://thatopen.github.io/engine_fragment/resources/worker.mjs",l=i.get(P);l.init(L);o.camera.controls.addEventListener("rest",()=>l.core.update(!0));o.onCameraChanged.add(e=>{for(const[,n]of l.list)n.useCamera(e.three);l.core.update(!0)});l.list.onItemSet.add(({value:e})=>{e.useCamera(o.camera.three),o.scene.three.add(e.object),l.core.update(!0)});const T=["https://thatopen.github.io/engine_components/resources/frags/school_arq.frag","https://thatopen.github.io/engine_components/resources/frags/school_str.frag"];await Promise.all(T.map(async e=>{var c;const n=(c=e.split("/").pop())==null?void 0:c.split(".").shift();if(!n)return null;const a=await(await fetch(e)).arrayBuffer();return l.core.load(a,{modelId:n})}));const s=i.get($);s.world=o;let t;const _=async()=>{t=s.create(),t.title="My Viewpoint",await t.updateCamera()},B=()=>{t&&t.takeSnapshot()},F=async()=>{t&&(console.log("Position before updating",t.position),await t.updateCamera(),console.log("Position after updating",t.position))},O=async()=>{if(!t)return;const e=new u;o.camera.controls.getPosition(e),console.log("Camera position before updating",e),await t.go();const n=new u;o.camera.controls.getPosition(n),console.log("Camera position before updating",n)};s.list.onItemSet.add(({value:e})=>{e.selectionComponents.add("3V$FMCDUfCoPwUaHMPfteW","1fIVuvFffDJRV_SJESOtCZ")});s.list.onItemSet.add(async({value:e})=>{const r=await i.get(D).getItems([{categories:[/DOOR/]}]),a=await l.modelIdMapToGuids(r);e.selectionComponents.add(...a)});const W=async()=>{if(!t)return;const e=t.selectionComponents,n=await t.getSelectionMap();console.log(e,n)},x=async()=>{if(!t)return;const e=await t.getSelectionMap();i.get(f).isolate(e)};s.list.onItemSet.add(({value:e})=>{i.get(U).create().viewpoints.add(e.guid)});const E=()=>{if(!t)return null;const e=s.snapshots.get(t.snapshot);return e||null};C.init();const[d,w]=g.create(e=>{const n=async({target:a})=>{a.loading=!0,await i.get(f).set(!0),a.loading=!1};let r=m`
