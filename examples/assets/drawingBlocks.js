import{B as O,J as M,t as j,u as N,R as U,p as _,m as S,a as g,b as F,n as $,V as h,P as x}from"./index-CihJyAGz.js";import{C as G,W,S as E,a as q,f as z,F as D}from"./index-Dy8ZicqY.js";import{T}from"./index-YMXcKuMf.js";import{B as V}from"./index-B-YSWD29.js";import"./index-gnb7d6C_.js";import"./three.tsl-DtvQaLlC.js";const i=new G,J=i.get(W),o=J.create();o.scene=new E(i);o.scene.setup();o.scene.three.background=null;const L=document.getElementById("container");o.renderer=new q(i,L);o.camera=new z(i);await o.camera.controls.setLookAt(48.213,33.495,-5.062,13.117,-1.205,22.223);i.init();const X=await D.getWorker(),l=i.get(D);l.init(X);o.camera.controls.addEventListener("update",()=>l.core.update());l.list.onItemSet.add(({value:e})=>{e.useCamera(o.camera.three),o.scene.three.add(e.object),l.core.update(!0)});l.core.models.materials.list.onItemSet.add(({value:e})=>{"isLodMaterial"in e&&e.isLodMaterial||(e.polygonOffset=!0,e.polygonOffsetUnits=1,e.polygonOffsetFactor=Math.random())});const Y=await fetch("https://thatopen.github.io/engine_components/resources/frags/school_arq.frag"),H=await Y.arrayBuffer();await l.core.load(H,{modelId:"school_arq"});const v=i.get(T),a=v.create(o);a.three.position.y=11.427046;const K=await fetch("https://thatopen.github.io/engine_components/resources/projections/projection.json").then(e=>e.json()),R=new O;R.setAttribute("position",new M(new Float32Array(K.positions),3));a.layers.create("projection",{material:new j({color:16711680})});const Q=new N(R);a.addProjectionLines(Q,"projection");const s=v.use(V);s.styles.set("COLUMN",{color:21964,textOffset:0,fontSize:0});s.styles.set("DOOR",{color:13386752,textOffset:0,fontSize:0});const y=[],u=.35,w=16;for(let e=0;e<w;e++){const t=e/w*Math.PI*2,n=(e+1)/w*Math.PI*2;y.push(Math.cos(t)*u,0,Math.sin(t)*u),y.push(Math.cos(n)*u,0,Math.sin(n)*u)}const A=new O;A.setAttribute("position",new M(new Float32Array(y),3));s.define("COLUMN",{lines:A});const r=.9,f=12,p=[];for(let e=0;e<f;e++){const t=e/f*(Math.PI/2),n=(e+1)/f*(Math.PI/2);p.push(Math.cos(t)*r,0,Math.sin(t)*r),p.push(Math.cos(n)*r,0,Math.sin(n)*r)}p.push(0,0,0,r,0,0);p.push(0,0,0,0,0,r);const I=new O;I.setAttribute("position",new M(new Float32Array(p),3));s.define("DOOR",{lines:I});let k=()=>{},c=null;s.onCommit.add(([{item:e}])=>{c=e.uuid,k()});let d="COLUMN",Z=0;const P=new U,B=new x,ee=e=>{const t=L.getBoundingClientRect();return new $((e.clientX-t.left)/t.width*2-1,-((e.clientY-t.top)/t.height)*2+1)},te=e=>{const t=new h(0,1,0).transformDirection(a.three.matrixWorld),n=new h().setFromMatrixPosition(a.three.matrixWorld);B.setFromNormalAndCoplanarPoint(t,n);const C=new h;return e.intersectPlane(B,C),a.three.worldToLocal(C)};L.addEventListener("click",e=>{P.setFromCamera(ee(e),o.camera.three);const t=te(P.ray);s.add(a,{blockName:d,position:t,rotation:Z,scale:1,style:d})});const oe=()=>{if(!c)return;const e=a.annotations.getBySystem(s).get(c);s.update(a,[c],{rotation:e.rotation+Math.PI/4})};_.init();const[b,ne]=S.create(e=>g`
