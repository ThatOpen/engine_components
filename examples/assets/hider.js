import{u as f,V as m,f as p,a as w,J as y}from"./index-BVinSk0X.js";import{C as h,W as C,S as I,a as v,O as $,F as S,H as k}from"./index-DBG1qVuX.js";const l=new h,R=l.get(C),a=R.create();a.scene=new I(l);a.scene.setup();a.scene.three.background=null;const H=document.getElementById("container");a.renderer=new v(l,H);a.camera=new $(l);await a.camera.controls.setLookAt(78,20,-2.2,26,-4,25);l.init();const x="https://thatopen.github.io/engine_fragment/resources/worker.mjs",i=l.get(S);i.init(x);a.camera.controls.addEventListener("rest",()=>i.core.update(!0));a.onCameraChanged.add(t=>{for(const[,e]of i.list)e.useCamera(t.three);i.core.update(!0)});i.list.onItemSet.add(({value:t})=>{t.useCamera(a.camera.three),a.scene.three.add(t.object),i.core.update(!0)});const B=["/resources/frags/school_arq.frag","/resources/frags/school_str.frag"];await Promise.all(B.map(async t=>{var s;const e=(s=t.split("/").pop())==null?void 0:s.split(".").shift();if(!e)return null;const n=await(await fetch(t)).arrayBuffer();return i.core.load(n,{modelId:e})}));const g=l.get(k),O=async t=>{const e={},c=t.map(n=>new RegExp(`^${n}$`));for(const[,n]of i.list){const s=await n.getItemsOfCategories(c),o=Object.values(s).flat();e[n.modelId]=new Set(o)}await g.isolate(e)},V=async t=>{const e={},c=t.map(n=>new RegExp(`^${n}$`));for(const[,n]of i.list){const s=await n.getItemsOfCategories(c),o=Object.values(s).flat();e[n.modelId]=new Set(o)}await g.set(!1,e)},L=async()=>{await g.set(!0)};f.init();const b=()=>p`
  ...
  `,u=m.create(()=>{const t=m.create(b),e=m.create(b);return p`
