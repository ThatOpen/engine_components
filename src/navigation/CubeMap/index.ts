...
export class CubeMap extends Component<HTMLDivElement> implements Updateable, Hideable {

    name: string = "CubeMap"
    enabled: boolean = true
    afterUpdate: Event<CubeMap> = new Event()
    beforeUpdate: Event<CubeMap> = new Event()
    private _cubeFaceClass = "flex justify-center font-bold hover:bg-ifcjs-200 hover:text-ifcjs-100 text-white select-none text-xl items-center cursor-pointer text-center bg-ifcjs-100 text-ifcjs-100 absolute w-[120px] h-[120px] border-2 border-solid border-ifcjs-120"
    ...
    constructor(components: Components) {
        ...
        rightFace.style.transform = "rotateY(-270deg) rotateX(180deg) translateZ(-60px)"
        ...
        leftFace.style.transform = "rotateY(-90deg) rotateX(180deg) translateZ(-60px)"
    ...
    private async _onFaceClick(orientation: CubeMapFace) {
        ...
        const endPoint = target.clone().add(this._faceOrientations[orientation].clone().multiplyScalar(intersection.distance))
