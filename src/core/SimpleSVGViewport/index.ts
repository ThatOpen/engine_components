...
export class SimpleSVGViewport extends Component<SVGElement> {
    ...
    private _viewport: SVGElement = document.createElementNS("http://www.w3.org/2000/svg", "svg")
    ...
    constructor(components: Components, config?: SVGViewportConfig) {
        ...
        this.setUI()
        this.enabled = false
        const viewerContainer = components.renderer.get().domElement.parentElement as HTMLElement
    ...
    private setUI() {
        const undoDrawingBtn = new Button(this._components, {materialIconName: "undo"})
        ...
        const redoDrawingBtn = new Button(this._components, {materialIconName: "redo"})
        ...
        const clearDrawingBtn = new Button(this._components, {materialIconName: "delete"})
        ...
        const strokeWidth = new SimpleRangeInput(this._components, {
            label: "Stroke width",
            name: "stroke-width",
            min: 2,
            max: 6,
        ...
        strokeWidth.onChange.on(e => {
            //@ts-ignore
            this.config = {strokeWidth: e.target.value}
        })

        const strokeColorInput = new SimpleColorInput(this._components, {
            label: "Stroke color",
        ...
        strokeColorInput.onChange.on(e => {
            //@ts-ignore
            this.config = {strokeColor: e.target.value}
        })

        const fillColorInput = new SimpleColorInput(this._components, {
            label: "Fill color",
        ...
        const settingsBtn = new Button(this._components, {materialIconName: "settings"})
        settingsBtn.onclick = () => {
        ...
        toolbar.addButton(settingsBtn, undoDrawingBtn, redoDrawingBtn, clearDrawingBtn)
