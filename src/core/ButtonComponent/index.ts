...
export class Button extends Component<HTMLButtonElement> implements Hideable, UIComponent {
    ...
    set visible(visible: boolean) { this.#visible = visible } // Not implemented yet.
    ...
    constructor(components: Components, options?: IButtonOptions) {
        ...
        this.domElement.onclick = e => {
            e.stopImmediatePropagation()
            //@ts-ignore
            if (!this.parent?.parent && this.components.ui) { this.components.ui.closeMenus() }
    ...
    get(): HTMLButtonElement { return this.domElement }
    ...
    #updateMenuPlacement() {
        ...
        if (this.parent?.position === "bottom") { placement = this.parent?.parent? "right" : "top" }
        if (this.parent?.position === "top") { placement = this.parent?.parent? "right" : "bottom" }
        if (this.parent?.position === "left") { placement = "right" }
        if (this.parent?.position === "right") { placement = "left" }
