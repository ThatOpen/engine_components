...
import { UI, UIComponent, TreeView, Component, Components, FragmentGroup, Fragments, SimpleUIComponent } from "../../"
...
export class PropertiesProcessor extends Component<IPropertiesList> implements UI {
    ...
    constructor(components: Components, config?: IComponentConfig) {
        super()
        this.components = components
        this._config = {...this._config, ...config}
        components.tools.onToolAdded.on( component => this.setFragments(component) )
        this.setFragments(components.tools.get("Fragments") as Fragments)
        this.setUI()
    }
    ...
    private setFragments(component: Component<any>) {
        if (this._fragments || !(component instanceof Fragments)) {return}
        const fragments = component
        const highlighterEvents = fragments.highlighter.highlightEvents
        ...
        highlighterEvents[this._config.selectionHighlighter]?.onHighlight.on( selection => {
            ...
            const modelID = fragments.list[fragmentID].mesh.parent?.uuid
            if (!modelID) { return }
    ...
    private processStoreys(model: FragmentGroup, props: IModelProperties) {
        ...
        spatialRelations.forEach( (rel: any) => {
            ...
            if (structure.type === webIfc.IFCBUILDINGSTOREY) {
                const { globalId, type, tag, ...storeyAttributes } = this.processAttributes(model, structure.expressID, { group: "Storey", prefix: "Storey" })
