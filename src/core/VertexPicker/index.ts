import { BufferAttribute, BufferGeometry, Intersection, Vector3 } from "three";
import { Component, Event, Updateable } from "../../base-types";
...
export class VertexPicker extends Component<Vector3|null> implements Updateable {
    ...
    constructor(components: Components, config?: Partial<VertexPickerConfig>) {
        ...
        marker.className = "rounded-full w-[15px] h-[15px] border-3 border-solid border-red-500"
    ...
    private getClosestVertex(intersects: Intersection) {
        ...
        vertices?.forEach((vertex) => {
            ...
            if (distance > closestDistance || distance > this._config.snapDistance) return;
            vertexFound = true;
            closestVertex = vertex;
            closestDistance = intersects.point.distanceTo(vertex);
        })
        return vertexFound ? closestVertex : this.config.showOnlyVertex? null : intersects.point
    }
    ...
    private getVertex(index: number, geom: BufferGeometry) {
        ...
        const vertices = geom.attributes.position as BufferAttribute;
        return new Vector3(
            vertices.getX(index),
            vertices.getY(index),
            vertices.getZ(index)
