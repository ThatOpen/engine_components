...
export class SimpleAngle extends Component<string> implements Hideable, Disposable, Updateable, UI {
  ...
  create() {
    if (!this._startPoint) {
      ...
      return null
    } else if (!this._endPoint && this._isHovering) {
  ...
  setAngleDegrees(pointB: Vector3) {
    ...
    if (this.currentAngle.tagDegrees) {
      ...
      if (this.currentAngle.insideArc) {this.removeFromScene(this.currentAngle.insideArc)}
  ...
  set visible(state) {
    ...
    for (const angle of this._angles) {
      if (angle.insideArc) {angle.insideArc.visible = this._visible}
      if (angle.tagDegrees) {angle.tagDegrees.get().visible = this._visible}
    }
  ...
  dispose() {
    ...
    for (const angle of this._angles) {
      if (angle.insideArc) {this.removeFromScene(angle.insideArc)}
      if (angle.tagDegrees) {this.removeFromScene(angle.tagDegrees.get())}
    }
